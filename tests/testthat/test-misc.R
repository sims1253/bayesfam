...
test_that("inverse-logit-link", {
  ...
  expect_eps(
    brms:::inv_logit(testset_invlogit),
    inv_logit(testset_invlogit),
    eps
  )
...
test_that("cloglog-link", {
  ...
  # check vector as argument returns vector with same results
  expect_eps(
    c(-9.21029, -2.250367, -0.005764308, 0.475885),
    cloglog(c(0.0001, 0.1, 0.63, 0.8)),
    eps
  )
  # check link against link from another package
...
test_that("inverse-cloglog-link", {
  ...
  # check vector as argument returns vector with same results
  expect_eps(
    c(0.04856801, 0.3077994, 0.6321206, 0.934012),
    inv_cloglog(c(-3, -1, 0, 1)),
    eps
  )
  # check against another package
  testset_invcloglog <- seq(from = -10, to = 5, length.out = n_testset)
  expect_eps(
    brms:::inv_cloglog(testset_invcloglog),
    inv_cloglog(testset_invcloglog),
    eps
  )
...
test_that("cauchit-link", {
  ...
  # check vector as argument returns vector with same results
  expect_eps(
    c(-3.077684, -0.3249197, 0, 1.376382),
    cauchit(c(0.1, 0.4, 0.5, 0.8)),
    eps
  )
  # check against another package
...
test_that("Softplus link-function", {
  ...
  # check vector as argument returns vector with same results
  expect_eps(
    c(-2.252168, 0.5413249, 3.981515, 100),
    softplus(c(0.1, 1, 4, 100)),
    eps
  )
  # check x approaching Inf on boundry of defined space
...
test_that("Inverse Softplus link-function", {
  ...
  # check vector as argument returns vector with same results
  expect_eps(
    c(4.53989e-05, 0.3132617, 1.313262, 100),
    inv_softplus(c(-10, -1, 1, 100)),
    eps
  )
  # check x approaching Inf on boundry of defined space
